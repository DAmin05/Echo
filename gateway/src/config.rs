...
/// Gateway configuration: where it listens and where the internal services are.
/// Cache tuning (threshold, TTL) lives in cache-svc; provider keys live in
/// provider-adapter. The gateway holds no secrets.
pub struct Config {
    pub port: u16,
